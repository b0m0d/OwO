...
async fn openapi_spec() -> Json<Value> {
    Json(serde_json::json!({
        ...
        "paths": {
            ...
            "/change-sets/{id}/accept": { "post": { "operationId": "changeSetAccept", "parameters": [path_param("id")], "responses": { "200": { "description": "接受变更（幂等重放返回 replayed:true 且零副作用）；accept 保留修改并解除该 ChangeSet 对最终 approved head 的阻断", "content": { "application/json": { "schema": { "type": "object", "additionalProperties": true, "properties": { "change_set": { "type": "object", "additionalProperties": true }, "replayed": { "type": "boolean" } } } } } }, "404": { "description": "change set not found" }, "409": { "description": "已终态（跨动作）或文件被用户再次修改（status=conflicted，不覆盖用户新内容）" } } } },
            "/change-sets/{id}/reject": { "post": { "operationId": "changeSetReject", "parameters": [path_param("id")], "responses": { "200": { "description": "拒绝变更（幂等重放返回 replayed:true 且零副作用）；reject 仅恢复该 ChangeSet 修改的文件（新建文件=删除恢复），恢复前逐文件比对当前哈希", "content": { "application/json": { "schema": { "type": "object", "additionalProperties": true, "properties": { "change_set": { "type": "object", "additionalProperties": true }, "replayed": { "type": "boolean" } } } } } }, "404": { "description": "change set not found" }, "409": { "description": "文件被用户再次修改（status=conflicted，不覆盖用户新内容）或已终态跨动作" } } } },
            "/change-sets/{id}/revert": { "post": { "operationId": "changeSetRevert", "parameters": [path_param("id")], "responses": { "200": { "description": "安全撤销（幂等重放返回 replayed:true 且零副作用）；仅恢复该 ChangeSet 修改的文件，恢复前比较当前文件哈希：=结果哈希→恢复、=基线哈希→已恢复跳过、否则 409 + status=conflicted", "content": { "application/json": { "schema": { "type": "object", "additionalProperties": true, "properties": { "change_set": { "type": "object", "additionalProperties": true }, "replayed": { "type": "boolean" } } } } } }, "404": { "description": "change set not found" }, "409": { "description": "文件被用户再次修改（status=conflicted，不覆盖用户新内容）或已终态跨动作" } } } },
