...
async fn openapi_spec() -> Json<Value> {
    Json(serde_json::json!({
        ...
        "paths": {
            ...
            "/teams": { "post": { "operationId": "workswarmCreateTeam", "requestBody": { "content": { "application/json": { "schema": { "type": "object", "properties": { "goal_id": { "type": "string" }, "objective": { "type": "string" }, "mode": { "type": "string", "enum": ["single", "team", "swarmflow"] }, "template_id": { "type": "string" }, "roles": { "type": "array", "items": { "type": "object" } }, "budget": { "type": "object" }, "human_policy": { "type": "string" }, "strategy": { "type": "string", "enum": ["auto", "single", "team"], "description": "五期组队策略（缺省 auto：按任务画像判定，不再盲目启用多 Agent）；未知值 400" } }, "required": ["objective"] } } } }, "responses": { "202": { "description": "team run created; background run loop drives phases；body 含 strategy_decision（组队决策：mode/roles/parallelism/budget_calls_total/reasons，供 UI 渲染组队理由与调用预算）" } } }, "get": { "operationId": "workswarmListTeams", "responses": { "200": { "description": "team run list; items = TeamRun + 进程内运行标志（R2 additive）", "content": { "application/json": { "schema": { "type": "object", "properties": { "teams": { "type": "array", "items": { "type": "object", "description": "TeamRun 字段（透传）+ 以下运行标志；additive 不改变既有字段", "properties": { "active": { "type": "boolean", "description": "运行循环正在执行阶段（人节点等待窗口 / 终态为 false）" }, "interrupted": { "type": "boolean", "description": "R2：磁盘 Running 但无活动运行 → 已识别为中断，等待显式 continue/retry 恢复" } } } } }, "required": ["teams"] } } } } } } },
