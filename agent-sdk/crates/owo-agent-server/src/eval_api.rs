...
//! builtin/builtin-demo 套件经 `owo_agent_eval_facade::run_suite` 执行）。
...
pub(super) async fn run_eval(
...
) -> Result<Json<owo_agent_eval_facade::EvalReport>, (StatusCode, String)> {
    let suite = match request.suite_id.as_str() {
        "builtin" | "builtin-demo" => owo_agent_eval_facade::builtin_suite(),
        _ => {
            return Err((
    ...
    let report = owo_agent_eval_facade::run_suite(provider, &model, &suite).await;
    Ok(Json(report))
}
